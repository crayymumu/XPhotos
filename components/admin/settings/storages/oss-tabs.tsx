...
import OSSEditSheet from '~/components/admin/settings/storages/oss-edit-sheet'
...
export default function OSSTabs() {
  const { data, error, isValidating, mutate } = useSWR('/api/v1/settings/oss-info', fetcher
    , { revalidateOnFocus: false })
  const { setOSSEdit, setOSSEditData } = useButtonStore(
    (state) => state,
  )
  ...
  return (
    <Space orientation="vertical" size={token.marginLG} style={{ width: '100%' }}>
      <Card
        title={<Typography.Title level={5} style={{ margin: 0 }}>{t('Config.ossTitle')}</Typography.Title>}
        extra={
          <Space>
            <Button
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  const res = await fetch('/api/v1/settings/validate-oss', { method: 'GET' })
                  ...
                  toast.success(`OSS 验证成功：${bucket} @ ${endpoint} — ${summary}`)
                } catch (e: any) {
                  toast.error(`OSS 验证失败：${e?.message || ''}`)
                }
              }}
            >
              验证配置
      ...
      {Array.isArray(data) && data.length > 0 && <OSSEditSheet />}
    </Space>
  )
}
